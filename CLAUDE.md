...
Current state: **modules 1–3** (User Management & Auth, Helpdesk & Ticketing, IT Asset Management) are implemented with both backend and frontend; **module 4 (Inventory Management)** has its backend. Modules 5–8 from [cloude.md](cloude.md) §2 (Daily Report, Project/Kanban, IPAM, Monitoring) are not yet built. When adding a module, follow the existing `apps/helpdesk`, `apps/assets`, or `apps/inventory` app as the template: subclass `apps.core.BaseModel` for domain models, reuse the auth RBAC permission classes, register the app in `LOCAL_APPS` ([backend/config/settings/base.py](backend/config/settings/base.py)), and mount its `urls` under `/api/v1/<module>/` in [backend/config/urls.py](backend/config/urls.py).
