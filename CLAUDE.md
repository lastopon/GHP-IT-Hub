...
Current state: **modules 1–5** (User Management & Auth, Helpdesk & Ticketing, IT Asset Management, Inventory Management, Daily Report & Checklist) are implemented with both backend and frontend; **module 6 (Project Management / Kanban)** has its backend. Modules 7–8 from [cloude.md](cloude.md) §2 (IPAM, Monitoring) are not yet built. When adding a module, follow the existing `apps/inventory`, `apps/dailyreport`, or `apps/projects` app as the template: subclass `apps.core.BaseModel` for domain models, reuse the auth RBAC permission classes, register the app in `LOCAL_APPS` ([backend/config/settings/base.py](backend/config/settings/base.py)), and mount its `urls` under `/api/v1/<module>/` in [backend/config/urls.py](backend/config/urls.py).
