...
Current state: **modules 1–4** (User Management & Auth, Helpdesk & Ticketing, IT Asset Management, Inventory Management) are implemented with both backend and frontend; **module 5 (Daily Report & Checklist)** has its backend. Modules 6–8 from [cloude.md](cloude.md) §2 (Project/Kanban, IPAM, Monitoring) are not yet built. When adding a module, follow the existing `apps/assets`, `apps/inventory`, or `apps/dailyreport` app as the template: subclass `apps.core.BaseModel` for domain models, reuse the auth RBAC permission classes, register the app in `LOCAL_APPS` ([backend/config/settings/base.py](backend/config/settings/base.py)), and mount its `urls` under `/api/v1/<module>/` in [backend/config/urls.py](backend/config/urls.py).
